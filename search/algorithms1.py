...
class CBSState:
    ...
    def compute_cost(self):
        ...
        for i in range(self._k):
            star_state= self._starts[i]
            goal_state= self._goals[i]
            temp_cost,self._paths[i]= AStar(self._map).search(star_state,goal_state,constraints=self._constraints[i])
            temp_cost += temp_cost
        self.set_cost(temp_cost)
            
        


        # for start_states,goal_states in zip(self._starts,self._goals):


        #     for goal_states in self._goals:
        #         print ("Start states : ",start_states)
        #         print ("Goal states : ",goal_states)
        #         temp_cost= AStar(self._map).search(start_states, goal_states)[0]
        #         print ("Temp cost : ",temp_cost)
        #         self._cost = self._cost+temp_cost
